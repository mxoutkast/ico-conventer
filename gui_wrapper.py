...
class IcoConverterGUI:
    ...
    def _setup_ui(self) -> None:
        ...
        # Handle macOS specific binding
        try:
            self.file_list.bind('<Command-a>', self._select_all_files)
        except tk.TclError:
            pass  # Ignored on non-macOS platforms
    ...
    def _select_all_files(self, event: Optional[tk.Event] = None) -> Optional[str]:
        ...
        Args:
            event: Optional tkinter event when triggered by a keyboard shortcut

        Returns:
            'break' if triggered by event to prevent default handling
        """
    ...
    def _remove_selected_files(self, event: Optional[tk.Event] = None) -> Optional[str]:
        """
