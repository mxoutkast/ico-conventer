...
class IcoConverterGUI:
    ...
    def _setup_ui(self) -> None:
        ...
        # Also bind macOS Command-a if possible, wrapped in try-except to be safe
        try:
    ...
    def _select_all_files(self, event: Optional[tk.Event] = None) -> str:
        ...
        Args:
            event: Optional event object (when called from binding)
    ...
    def _remove_selected_files(self, event: Optional[tk.Event] = None) -> None:
        ...
        if not selected_items:
            # Only show message if called via button, not key press (to avoid spam)
            if event is None:
