...
def test_file_list_component():
    ...
    try:
        ...
        with open('gui_wrapper.py', 'r') as f:
            ...
            assert "Remove Selected (Del)" in content, "Remove button text not updated"
            print("  ✓ Remove Selected button text updated")

            # Check for keyboard shortcuts
            assert "bind('<Delete>'" in content, "Delete key binding not found"
            assert "bind('<BackSpace>'" in content, "Backspace key binding not found"
            print("  ✓ Keyboard shortcuts (<Delete>, <BackSpace>) configured")
