...
def test_file_list_component():
    ...
    try:
        ...
        with open('gui_wrapper.py', 'r') as f:
            ...
            assert "self.file_list.bind('<BackSpace>', self._remove_selected_files)" in content, "Backspace binding not found"
            assert "self.file_list.bind('<Control-a>', self._select_all_files)" in content, "Ctrl-A binding not found"
            assert "self.file_list.bind('<Command-a>', self._select_all_files)" in content, "Cmd-A binding not found"
            print("  ✓ Keyboard bindings exist")
